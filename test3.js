...
async function main() {
  try {
    ...
    console.log("\nTesting: setup with generationConfig only");
    await testWebSocket(token.name, { generationConfig: { responseModalities: ['AUDIO'] } });
    
    console.log("\nTesting: setup with ALL fields but exactly matching constraints");
    await testWebSocket(token.name, { 
      model: "models/gemini-3.1-flash-live-preview",
      systemInstruction: { parts: [{text: "You are a fun assistant."}]} 
    });
        ...
        ws.on('open', () => {
          ...
          setTimeout(() => {
              if (ws.readyState === WebSocket.OPEN) {
                ws.send(JSON.stringify({ realtimeInput: { text: "Hello there!" } }));
              }
          }, 1000);
        });
